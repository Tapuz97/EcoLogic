...
function initAdminNavActive(){ const path = location.pathname.split('/').pop(); $$('.admin-nav a').forEach(a => { const href = a.getAttribute('href'); if (href === path) a.setAttribute('aria-current','page'); else a.removeAttribute('aria-current'); }); }
...
(function initIcons(){
  ...
  const MAP = { house:{base:'house'}, doc:{base:'doc'}, export:{base:'export'}, chart:{base:'chart'}, logout:{base:'logout'}, user:{base:'user'} };
  ...
  function apply(img){ const name = img.dataset.icon; if (!name) return; const isActive = img.closest('a')?.getAttribute('aria-current') === 'page'; img.src = ICON_BASE + fileFor(name, { active:isActive, theme:getTheme() }); }
  document.querySelectorAll('img[data-icon]').forEach(apply); window.addEventListener('ecologic:theme-change', () => document.querySelectorAll('img[data-icon]').forEach(apply));
})();
